...
def compress_disk_list(disk_list, num_files):
    ...
    def get_next_file_block(disk_list, i):
        ...
        while disk_list[block_end] == ".":
            block_end -= 1
        block_end += 1
        # get start of block
        block_start = block_end - 1
        while disk_list[block_start] == disk_list[block_end-1]:
            block_start -= 1 
        block_start += 1
...
if __name__ == "__main__":
    ...
    for i, id in enumerate(res_disk_list):
        if id != ".":
            res += i * id
    print(res) # 6636716706623 answer too high, but getting test input right on 2858... 
